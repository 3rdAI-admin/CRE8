...
// Typed environment bindings matching wrangler.toml / worker-configuration.d.ts.
// Reason: the global Env interface generated by `wrangler types` does not resolve
// via declaration merging with the bundled runtime types, so we declare the
// properties explicitly here for compile-time safety.
...
export type ExtendedEnv = WorkerEnv & { OAUTH_PROVIDER: OAuthHelpers };
...
export function createSuccessResponse(message: string, data?: unknown): McpResponse {
  let text = `**Success**\n\n${message}`;
  if (data !== undefined) {
...
export function createErrorResponse(message: string, details?: unknown): McpResponse {
  let text = `**Error**\n\n${message}`;
